...
let SHOP_WORK_DAYS = {'sun': false, 'mon': false, 'tue': false, 
    'wed': false, 'thu': false, 'fri': false, 'sat': false} as any

let OPENS_AT = document.getElementById("start-work") as HTMLInputElement | null
let CLOSES_AT = document.getElementById("end-work") as HTMLInputElement | null
let WORKDAY_CHECKBOXES = document.getElementById("day-work-select")?.querySelectorAll('input[type=checkbox]') as NodeListOf<HTMLInputElement> | null
let RADIO_FIRST_WORKDAY = document.getElementById("first-day-radio")?.querySelectorAll('input[type=radio]') as NodeListOf<HTMLInputElement> | null
...
async function LoadSettings() {
    ...
    if (OPENS_AT) OPENS_AT.value = shop_opensAt
    if (CLOSES_AT)  CLOSES_AT.value = shop_closesAt

    // radio de primeiro dia da semana
    if (RADIO_FIRST_WORKDAY) RADIO_FIRST_WORKDAY[shop_firstWeekDay].checked = true
    
    // checkbox de dias de trabalho
    Object.values(SHOP_WORK_DAYS).forEach((workDay: boolean, index) => {
        if (WORKDAY_CHECKBOXES) WORKDAY_CHECKBOXES[index].checked = workDay;
    });
}
...
/* -------------------------------------------------------------------------- */
const CONFIGURATION_FORM = document.getElementById("configuration-form") as HTMLFormElement;
