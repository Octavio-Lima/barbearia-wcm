...
export function DisplayPopup(showWindows, windowIndex, resetForm = false) {
    ...
    // reset forms
    if (resetForm) {
        const ADD_ENTRY_FORM = document.getElementById("add-entry-form");
        if (ADD_ENTRY_FORM)
...
function RefreshTotalProfit_worker(worker_id, worker_name) {
    ...
    createByList.forEach(by => {
        if (by.textContent == worker_name) {
            ...
            if (element && myEntryList)
                myEntryList.push(element);
        }
    });
    ...
    if (el_workerProfit)
        el_workerProfit.textContent = FloatToMoney(sum);
}
/* -------------------------------------------------------------------------- */
...
if (ACCESS_NAME)
    ACCESS_NAME.textContent = accessName;
/* -------------------------------------------------------------------------- */
