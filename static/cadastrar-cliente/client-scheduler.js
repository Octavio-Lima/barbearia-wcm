...
CALENDAR_DAY_LIST.forEach(day => {
    day.addEventListener("click", () => {
        ...
        schedule_day = parseInt(day.innerText);
...
function SelectServiceEntry(entry) {
    ...
    let service_value = parseInt(entry.querySelector('[data-value]').getAttribute('data-value'));
    ...
    if (entry.classList.contains("selected")) {
        ...
        let entryToRemove = schedule_services.indexOf(serviceList[service_id]);
        schedule_services.splice(entryToRemove, 1);

        // Subtrai tempo de duração e valor
        schedule_duration = TIME.subtract(schedule_duration, service_duration);
        ...
        schedule_duration = TIME.sum(schedule_duration, service_duration);
        schedule_totalPrice += service_value;
    }
...
export async function UpdateSchedule() {
    ...
    scheduleBlockList.forEach(schedule => {
        ...
        let scheduleStart = scheduleEnd - schedule_duration - 1;
...
function SelectScheduleEntry(entry) {
    if (!entry.classList.contains("unavailable")) {
        ...
        // Selecionar
        entry.classList.add("selected");
        schedule_time = entry.querySelector(".entry-time").innerText;
    }
}
...
export function UpdateClientSummaryInfo() {
    ...
    SCHEDULE_DATE.innerText = schedule_dateTitle;
    SCHEDULE_SUMMARY.innerText = `${schedule_time.replace(':', 'H ')} - ${schedule_duration.replace(":", "H")}`;
