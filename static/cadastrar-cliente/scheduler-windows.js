...
import { schedule_day, UpdateCalendar, UpdateServiceTable, schedule_services, schedule_time, UpdateSchedule, isNextMonth, UpdateClientSummaryInfo, ClearClientForms } from "./client-scheduler";
...
function GoToWindowIndex(index, ignoreRequirements = false) {
    if (CheckWindowRequirements() || ignoreRequirements) {
        ...
        if (SUB_MODAL_LIST)
            SUB_MODAL_LIST.forEach(page => { if (page) {
