...
import { schedule_day, UpdateCalendar, UpdateServiceTable, schedule_services, schedule_time,
    UpdateSchedule, isNextMonth, UpdateClientSummaryInfo, ClearClientForms } from "./cadastrar-cliente-agendar.js";
...
// * Sub Modal
const SUB_MODAL_ELEMENT_LIST = [ // Agrupar todas as janelas em uma array só
...
export class Modal {
    ...
    static DisplayModal(display = true) {
        ...
        this.GoToWindowIndex(0, true);
    }

    // Acessar página informada da janela 
    GoToWindowIndex(index: subModalName, ignoreRequirements = false) {
        if (CheckWindowRequirements() || ignoreRequirements) {
            // esconder todas os avisos e submodais
            if (SUB_MODAL_ELEMENT_LIST)
            SUB_MODAL_ELEMENT_LIST.forEach(page => { if (page) { page.classList.add('d-none');} })
            warningMsg.forEach(message => { if (message) { message.classList.add('d-none');} })
...
function UpdateWindowInfo(windowIndex: subModalName) {
    if (windowIndex == "calendario") { UpdateCalendar(isNextMonth); } else
    if (windowIndex == "information") { } else
    if (windowIndex == "services") { UpdateServiceTable(); } else
    if (windowIndex == "schedule") { UpdateSchedule(); } else
    if (windowIndex == "overview") { UpdateClientSummaryInfo(); }
}
...
CONFIRM_BUTTON_LIST.forEach(button => {
    button.addEventListener("click", () => {
        ...
        if (buttonAttribute)
        Modal.GoToWindowIndex(parseInt(buttonAttribute));
    })
});
...
function CheckWindowRequirements() {
    ...
    if (currentSubModal == 1) { if (CheckClientForm()) { return true; } } // conferir formulario do cliente
    if (currentSubModal == 2) { if (CheckSelectedServices()) { return true; } }  // conferir selecionamento do serviço / produto
...
function CheckClientForm() {
    ...
    let clientName = (document.querySelector("#input-client-name") as HTMLInputElement | null)?.value;
    let clientInstagram = (document.querySelector("#input-client-instagram") as HTMLInputElement | null)?.value;
    let clientEmail = (document.querySelector("#input-client-email") as HTMLInputElement | null)?.value;
    let clientPhone = (document.querySelector("#input-client-phone") as HTMLInputElement | null)?.value;
...
function CheckScheduling(index: number) {
