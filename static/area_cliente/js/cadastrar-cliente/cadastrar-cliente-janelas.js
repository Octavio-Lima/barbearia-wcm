...
export class Modal {
    ...
    GoToWindowIndex(index, ignoreRequirements = false) {
...
function UpdateWindowInfo(windowIndex) {
    if (windowIndex == "calendario") {
        UpdateCalendar(isNextMonth);
    }
    else if (windowIndex == "information") { }
    else if (windowIndex == "services") {
        UpdateServiceTable();
    }
    else if (windowIndex == "schedule") {
        UpdateSchedule();
    }
    else if (windowIndex == "overview") {
