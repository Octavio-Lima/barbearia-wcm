...
let BARBER_LIST = document.querySelector("#barber-select");
/* Modal Adicionar novo Serviço */
let CONFIRM_BUTTON = document.getElementById("btn_confirm-new-entry");
let NEW_ENTRY_NAME = document.getElementById("inp_new-entry-name");
let NEW_ENTRY_VALUE = document.getElementById("inp_new-entry-value");
let NEW_ENTRY_DURATION = document.getElementById("inp_new-entry-length");
...
function CreateTableEntry(name, value, length) {
    ...
    if (SERVICE_TABLE)
        SERVICE_TABLE.appendChild(entry);
}
...
if (CONFIRM_BUTTON)
    CONFIRM_BUTTON.addEventListener("click", () => { CreateTableEntry(NEW_ENTRY_NAME.value, NEW_ENTRY_VALUE.value, NEW_ENTRY_DURATION.value); });
if (SHOW_MODAL)
    SHOW_MODAL.addEventListener("click", () => { if (MODAL_NEW_ENTRY)
        MODAL_NEW_ENTRY.classList.toggle("d-none"); });
if (HIDE_MODAL)
    HIDE_MODAL.addEventListener("click", () => { if (MODAL_NEW_ENTRY)
        MODAL_NEW_ENTRY.classList.toggle("d-none"); });
if (CONFIRM_NEW_ENTRY)
    CONFIRM_NEW_ENTRY.addEventListener("click", () => { if (MODAL_NEW_ENTRY)
        MODAL_NEW_ENTRY.classList.toggle("d-none"); });
...
if (SAVE_CHANGES)
    SAVE_CHANGES.addEventListener("click", function () {
        ...
        let newServiceList;
        if (SERVICE_TABLE)
            SERVICE_TABLE.childNodes.forEach(serviceEntry => {
