...
let OPENS_AT = document.querySelector("#start-work") as HTMLInputElement
let CLOSES_AT = document.querySelector("#end-work") as HTMLInputElement
let chk_WorkDays = document.querySelector("#day-work-select")?.querySelectorAll('input[type=checkbox]') as NodeListOf<HTMLInputElement>
let rad_FirstWorkDay = document.querySelector("#first-day-radio")?.querySelectorAll('input[type=radio]') as NodeListOf<HTMLInputElement>
...
async function GetShopList() {
    ...
    if (SHOP_NAME) SHOP_NAME.innerHTML = shopName.shopName
...
async function LoadSettings() {
    ...
    if (shop_firstWeekDay) rad_FirstWorkDay[shop_firstWeekDay].checked = true
...
if (SAVE_CHANGES) SAVE_CHANGES.addEventListener("click", async () => {
    if (!hasUpdatedTable) 
        return
...
function GetFirstWorkDayNumber(dayName: string) {
    let index = 0;
    for (const [key] of Object.entries(shop_workDays)) {
        if (dayName == key) {
            return index;
