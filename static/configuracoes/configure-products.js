...
function CreateTableEntry(name, value, quantity) {
    ...
    if (SERVICE_TABLE)
        SERVICE_TABLE.appendChild(entryRow);
}
/* Modal Adicionar novo Serviço */
const CONFIRM_BUTTON = document.querySelector("#btn_confirm-new-entry");
const input_newEntryName = document.querySelector("#inp_new-entry-name");
const input_newEntryValue = document.querySelector("#inp_new-entry-value");
const input_newEntryQuantity = document.querySelector("#inp_new-entry-quantity");
if (CONFIRM_BUTTON)
...
if (BUTTON_SHOW_MODAL)
    BUTTON_SHOW_MODAL.addEventListener("click", () => {
        if (MODAL_NEW_ENTRY)
            MODAL_NEW_ENTRY.classList.toggle("d-none");
    });
if (BUTTON_HIDE_MODAL)
    BUTTON_HIDE_MODAL.addEventListener("click", () => {
        if (MODAL_NEW_ENTRY)
            MODAL_NEW_ENTRY.classList.toggle("d-none");
    });
if (BUTTON_CONFIRM_NEW_ENTRY)
    BUTTON_CONFIRM_NEW_ENTRY.addEventListener("click", () => {
        if (MODAL_NEW_ENTRY)
            MODAL_NEW_ENTRY.classList.toggle("d-none");
    });
...
if (BUTTON_SAVE_CHANGES)
    BUTTON_SAVE_CHANGES.addEventListener("click", async () => {
        let entries;
        let serviceElements = SERVICE_TABLE?.querySelectorAll('tr');
        if (serviceElements)
            serviceElements.forEach(service => {
                let nodeList = [
                    service.childNodes[0].firstChild,
                    service.childNodes[1].firstChild,
                    service.childNodes[2].firstChild
                ];
